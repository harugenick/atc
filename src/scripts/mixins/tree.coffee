...
define ['backbone'], (Backbone) ->
  ...
  treeMixin =
    ...
    _initializeTreeHandlers: (options) ->
      throw new Error('BUG: Missing constructor options') if not options
      throw new Error('BUG: Missing root') if not options.root
    ...
    getChildren: () ->
      @_tree_children or throw new Error('BUG! This node has no children. Call _initializeTreeHandlers ?')
    ...
    removeChild: (model) ->
      ...
      if not children.contains(model) or children.get(model.id)
        throw new Error('BUG: child is not in this node')
